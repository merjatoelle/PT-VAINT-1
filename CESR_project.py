...
time_start = pd.to_datetime(['2010-01-01'   , '2011-01-01'   , '2012-01-01'   , '2013-01-01'   , '2014-01-01'   , '2015-01-01'   ])
time_stop  = pd.to_datetime(['2010-12-31 23', '2011-12-31 23', '2012-12-31 23', '2013-12-31 23', '2014-12-31 23', '2015-12-31 23'])
...
for  index in range(len(time_start)):   
    ...
    #GLEAM data v3.5a        
    ep_v35a = df_v35a['Ep'][daily_period].resample(ts).mean()                  
    et_v35a = df_v35a['Et'][daily_period].resample(ts).mean()                  
    ...
    for k in range(len(clm_name)):       
        ...
        if clm_name[k] in ('ALHFL_BS', 'ALHFL_PL' , 'ZTRALEAV', 'ZTRANG'   ,   # Plot data parameters
            ...
            if ldaily == daily_time_step:               
                plot4par = vis.dplots(ax, name_1[k], name_2[k], name_3  , date_ind ,  
                                          nst, l_p ,  y_min[k], y_max[k], y_step[k],
                                          time_start[index]   , time_stop[index]   )                                     
            else:              
                plot4par = vis.lplots(ax, name_1[k], name_2[k], date_ind , nst, 
